/**
 * thompson.js — Thompson Cigar scraper with detailed logging
 */
...
async function searchThompson(query) {
  ...
  try {
    ...
    const diagnostics = await page.evaluate(() => {
      ...
      const productLinks = allLinks.filter(a => a.href.includes("/product/") || a.href.includes("/p/"));
    ...
    console.log(`[Thompson] diagnostics:`, JSON.stringify(diagnostics));
    ...
    console.log(`[Thompson] raw items found: ${items.length}`);
    ...
    console.log(`[Thompson] final products: ${products.length}`);
    return products;
  } catch (err) {
