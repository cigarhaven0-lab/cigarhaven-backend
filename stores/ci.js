...
async function searchCI(query) {
  ...
  try {
    ...
    const items = await page.evaluate(() => {
      ...
      // CI individual product pages contain /p/ followed by a product ID
      const anchors = Array.from(document.querySelectorAll("a[href*='/p/']")).filter(a => {
        const href = a.href || "";
        // Must have something after /p/ — actual product slug, not just /p/
        return /\/p\/[a-zA-Z0-9\-_]+/.test(href);
      });
    ...
    for (const item of items) {
      ...
      // CI sometimes shows "As low as $X" — prefer that as the displayed price
